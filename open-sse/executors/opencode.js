...
export class OpenCodeExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true) {
    ...
    // OpenCode Zen's free-tier IP rate limiter keys on the x-real-ip request
    // header (rawIp = headers.get("x-real-ip") ?? "" → ip = "unknown"), so
    // requests without it share ONE global quota bucket with every other
    // anonymous proxy client worldwide — exhausting it burns everyone.
